...
// --- Icon Imports ---
import { 
  FaArrowLeft, FaExclamationTriangle, FaCheckCircle, FaSpinner, FaPlusCircle
} from 'react-icons/fa';
...
export default function CreateTeacherPage() {
  ...
  const [form, setForm] = useState({ name: "", gender: "", subjects: "", phoneNumber: "", idNumber: "", gmailId: "", password: "" });
  ...
  const submit = async (e) => {
    ...
    const payload = {
      ...
      gmailId: form.gmailId.trim().toLowerCase(),
      password: form.password,
    };
    if (!payload.name || !payload.gmailId || !payload.password || payload.subjects.length === 0) {
      setErr("Name, Gmail, Password and at least one Subject are required.");
      return;
    }
    try {
      setSubmitting(true);
      const t = await createTeacher(payload);
      setMsg(`Teacher created: ${t?.name || payload.name} (${payload.gmailId})`);
      setForm({ name: "", gender: "", subjects: "", phoneNumber: "", idNumber: "", gmailId: "", password: "" });
    } catch (e2) { setErr(e2.message || "Failed to create teacher"); }
          ...
          <form onSubmit={submit} style={styles.card}>
            <div style={dynamicStyles.grid}>
              ...
              <Field label="Subjects * (comma separated)" span>
                <StyledInput name="subjects" placeholder="e.g., Maths, Physics" value={form.subjects} onChange={onChange} />
              </Field>
              <Field label="Phone Number"><StyledInput name="phoneNumber" value={form.phoneNumber} onChange={onChange} /></Field>
              <Field label="ID Number"><StyledInput name="idNumber" value={form.idNumber} onChange={onChange} /></Field>
              <Field label="Gmail ID * (for login)"><StyledInput type="email" name="gmailId" value={form.gmailId} onChange={onChange} /></Field>
              <Field label="Temporary Password *"><StyledInput type="text" name="password" value={form.password} onChange={onChange} /></Field>
            </div>
            ...
            <div style={styles.buttonGroup}>
              ...
              <StyleButton type="submit" disabled={submitting} baseStyle={styles.btnPrimary} hoverStyle={styles.btnPrimaryHover}>
                {submitting ? <FaSpinner className="spinner"/> : <><FaPlusCircle /> Create</>}
              </StyleButton>
            </div>
          </form>
        </div>
