...
export async function fetchSessionSummary(sessionId) {
  ...
  return data; // { sessionId, present, total }
}

export async function fetchSessionById(id) {
  const headers = teacherHeadersOrThrow();
  const { data } = await axios.get(`/api/teacher/sessions/${id}`, { headers });
  return data; // { id, flow: ["WIFI","GEO","FACE","QR"], ... }
}

/** SSE: live stream of attendance marks on the teacher session page */
export function openSessionEventStream(sessionId, handlers = {}) {
  // EventSource cannot send custom headers; make sure the backend endpoint is public.
  let base = axios?.defaults?.baseURL;
  if (!base) {
    const env = (process.env.REACT_APP_API_BASE_URL || "").trim();
    base = env || `${window.location.protocol}//${window.location.host}`;
  }
  base = base.replace(/\/+$/, "");
