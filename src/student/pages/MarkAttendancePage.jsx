...
const styles = {
  ...
  checkRow: {
    ...
    alignItems: "center",
    gap: "12px",
    label: { width: "70px", fontWeight: 500, color: "var(--text-primary)" },
    dot: { width: "10px", height: "10px", borderRadius: "50%" },
    detail: { color: "var(--text-secondary)", fontSize: "14px" }
  }
};
...
function StyledInput(props) {
  ...
  const combinedStyle = focus
    ? { ...styles.input, ...styles.inputFocus, ...readOnlyStyle }
    : { ...styles.input, ...readOnlyStyle };
  ...
  const Row = ({ label, ok, detail }) => (
    <div style={styles.checkRow}>
      ...
      <span
        style={{
          ...styles.checkRow.dot,
          backgroundColor: ok ? "var(--success-color)" : "var(--danger-color)"
        }}
      />
...
function Stepper({ currentStep, flow }) {
  ...
  const progressLineActiveStyle = {
    ...progressLineStyle,
    background: "var(--primary-color)",
    width: `${(activeIndex / (visibleSteps.length - 1)) * 100}%`,
...
function useCurrentLocation() {
  ...
  const run = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => setMsg(`Location error: ${error.message}`),
...
export default function MarkAttendancePage() {
  ...
  const loadMeta = async () => {
    if (!sessionId) {
      setErr("Please enter a session ID.");
      return;
    }
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      if (!video) {
        setErr("Camera view unavailable.");
        setCamOn(false);
        return;
      }
  ...
  const startQrScan = useCallback(async () => {
    ...
    try {
      ...
      if (!video) {
        setScanErr("QR scanner view unavailable.");
        setScanning(false);
        return;
      }
      ...
      if (e?.name === "NotAllowedError") {
        ...
        setScanErr("No suitable camera found.");
      } else {
        setScanErr("Could not start QR scanner: " + (e?.message || e));
      }
      stopQr();
    }
  ...
  const handleSwitchCamera = async () => {
    ...
    try {
      if (qrScannerRef.current) {
        await qrScannerRef.current.setCamera(nextMode);
      }
    } catch {
      setScanErr("Could not switch camera.");
    }
  };
    ...
    try {
      ...
      if (needsWifi) {
        ...
        if (w.ok) newChecks.WIFI = { ok: true, ...w.data };
          ...
          newChecks.WIFI = { ok: false, ...w.error };
          wifiOk = false;
        }
      }

      if (needsGeo) {
        const g = await checkGeoApi({
          sessionId: Number(sessionId),
          studentLat: Number(lat),
          studentLng: Number(lng)
        });

        if (g.ok) newChecks.GEO = { ok: true, ...g.data };
        else {
          newChecks.GEO = { ok: false, ...g.error };
          geoOk = false;
        }
      }
        ...
        <div style={styles.container}>
          <header style={dynamicStyles.header}>
            ...
            <div style={styles.headerActions}>
              ...
              <StyleButton
                baseStyle={styles.btnDanger}
                hoverStyle={styles.btnDangerHover}
                onClick={onLogout}
              >
                <FaSignOutAlt /> Logout
              </StyleButton>
          ...
          {step === STEP.PICK_SESSION && (
            <div style={styles.card}>
              ...
              <p style={styles.infoText}>
                Please enter the numeric Session ID provided by your instructor to begin.
              </p>
              ...
              <div style={styles.buttonGroup}>
                ...
                >
                  {loading ? <FaSpinner className="spinner" /> : "Load Session"}
                </StyleButton>
              </div>
            </div>
          )}
          ...
          {step === STEP.WIFI_GEO && (
            <div style={styles.card}>
              ...
              <p style={styles.infoText}>
                Your public IP address and geolocation are being collected for verification.
              </p>
              ...
              <div style={dynamicStyles.gridContainer}>
                ...
                <Field label="Latitude">
                  <StyledInput value={lat} readOnly />
                </Field>

                <Field label="Longitude">
                  <div style={styles.inputGroup}>
                    <StyledInput value={lng} readOnly />
                    <StyleButton
                      baseStyle={styles.btnSecondary}
                      hoverStyle={styles.btnSecondaryHover}
                      onClick={getLoc}
              ...
              <div style={{ ...styles.infoText, marginTop: "16px", fontSize: "13px" }}>
                {geoMsg}
              </div>
              ...
              <div style={styles.buttonGroup}>
                <StyleButton
                  baseStyle={styles.btnSecondary}
                  hoverStyle={styles.btnSecondaryHover}
                  onClick={() => setStep(STEP.PICK_SESSION)}
                >
                  Back
                </StyleButton>
                ...
                <StyleButton
                  baseStyle={styles.btnPrimary}
                  hoverStyle={styles.btnPrimaryHover}
                  onClick={runWifiGeoChecks}
                  disabled={!canContinueWifi || loading}
                >
                  {loading ? <FaSpinner className="spinner" /> : "Verify Environment"}
                </StyleButton>
              </div>
            </div>
          )}
          ...
          {step === STEP.FACE && (
            <div style={styles.card}>
              ...
              <p style={styles.infoText}>
                Please capture a clear, forward-facing selfie for verification.
              </p>
              ...
              {camMsg && (
                <div style={{ ...styles.infoText, minHeight: "18px", margin: "16px 0 8px 0" }}>
                  {camMsg}
                </div>
              )}
              ...
              <video
                ...
                muted
                playsInline
                style={{ ...styles.videoPlayer, display: camOn ? "block" : "none" }}
              />
              ...
              {selfiePreview && (
                <img src={selfiePreview} alt="selfie preview" style={styles.selfiePreview} />
              )}
              ...
              <div style={styles.buttonGroup}>
                <StyleButton
                  ...
                  onClick={() => {
                    stopCamera();
                    setStep(orderedSteps[orderedSteps.indexOf(STEP.FACE) - 1] ?? STEP.PICK_SESSION);
                  }}
                >
                  Back
                </StyleButton>
                ...
                >
                  {loading ? <FaSpinner className="spinner" /> : "Verify Face"}
                </StyleButton>
              </div>
            </div>
          )}
          ...
          {step === STEP.QR && (
            <div style={styles.card}>
              ...
              <p style={styles.infoText}>
                Scan the QR code from the presenter's screen, or paste the token manually.
              </p>

              <div style={dynamicStyles.gridContainer2}>
                <Field label="QR Token">
                  <StyledInput
                    placeholder="Paste token here if needed"
                    value={qrToken}
              ...
              {scanErr && (
                <p style={{ ...styles.errorBox, marginTop: "24px" }}>
                  <FaExclamationTriangle /> {scanErr}
                </p>
              )}
              ...
              <div style={styles.buttonGroup}>
                <StyleButton
                  ...
                  onClick={() => {
                    stopQr();
                    setStep(orderedSteps[orderedSteps.indexOf(STEP.QR) - 1] ?? STEP.PICK_SESSION);
                  }}
                >
                  Back
                </StyleButton>
                ...
                >
                  {loading ? <FaSpinner className="spinner" /> : "Verify & Submit"}
                </StyleButton>
              </div>
            </div>
          )}
          ...
          {step === STEP.DONE && (
            <div style={styles.card}>
              <div style={{ textAlign: "center", padding: "32px 0" }}>
                <FaCheckDouble
                  size={48}
                  style={{ color: "var(--success-color)", marginBottom: "24px" }}
                />
                <h2 style={styles.sectionTitle}>Attendance Marked</h2>
                <p style={styles.infoText}>
                  Your attendance has been successfully recorded. You can now safely close
                  this page or return to the dashboard.
                </p>
